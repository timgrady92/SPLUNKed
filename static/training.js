...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initTrainingOnReady);
} else {
    initTrainingOnReady();
}
...
function initSearch() {
    ...
    input.addEventListener('input', (e) => {
        trainingSearch = e.target.value.toLowerCase().trim();
        if (clearBtn) clearBtn.style.display = trainingSearch ? 'block' : 'none';
        renderActiveTab();
    });
...
function openGuideModal(guide) {
    ...
    // Add copy handlers for SPL blocks
    body.querySelectorAll('.spl-block').forEach(block => {
        const code = block.querySelector('.spl-code code, .spl-code');
        if (!code) return;
        ...
        if (!block.querySelector('.spl-copy')) {
